...
hugepages_size=$(sysctl -a 2> /dev/null | grep vm.nr_hugepages | cut -d ' ' -f 3 | head -n 1)
readonly hugepages_size
...
if [[ -z $hugepages_size || $hugepages_size -lt 512 ]] ; then
    ...
    new_hugepages_size=$(sysctl -a 2> /dev/null | grep vm.nr_hugepages | cut -d ' ' -f 3 | head -n 1)
    readonly new_hugepages_size
